...
public abstract class AbstractClient extends AbstractEndpoint implements Client {
    ...
    /**
     * 重新连接 warning 的间隔.(多少次之后，warning一次) //for test
     */
    ...
    public AbstractClient(URL url, ChannelHandler handler) throws RemotingException {
        ...
        try {
            ...
            // 若是连接失败 RemotingException ，若开启了 启动时检查 ，则调用 #close() 方法，进行关闭。
            ...
            Runnable connectStatusCheckCommand = new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        // 线上因为中间件组件，打印了太多的日志，结果整个 JVM 崩了。特别在网络场景 + 大量“无限”重试的场景，特别容易打出满屏的日志。
    ...
    /**
     * 该方法，是因为实现 Channel 接口( Client 实现 Channel 接口 )，所以需要实现的。
     * 实际方法内部，调用的是 channel 对象，进行判断。其它实现 Channel 的方法，也是这么处理的，例如 #getAttribute(key) 等方法。
     * @return
     */
    @Override
    ...
    protected void connect() throws RemotingException {
        ...
        connectLock.lock();
        try {
            // 调用 #isConnected() 方法，判断连接状态。若已经连接，就不重复连接。
            if (isConnected()) {
                return;
            }
            // 初始化重连线程
